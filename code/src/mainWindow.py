from PyQt5.QtWidgets import QApplication, QLabel, QPushButton, QVBoxLayout, QWidget, QGridLayout, QMainWindow, \
    QAction, QListWidgetItem, QPlainTextEdit, QCheckBox, QScrollArea, qApp, QGraphicsScene, QGraphicsView, \
    QGraphicsProxyWidget, QGraphicsRectItem
from PyQt5.QtGui import QPen, QLinearGradient, QColor, QIcon, QGradient, QFont
...
class MainWindow(QMainWindow):
    ...
    def create_warning_display(self):
        ...
        self.warning_label.setStyleSheet("background:rgb(175, 201, 182); color:rgb(0,100,0);")
    ...
    def create_checkbox_scroll(self):
        scroll = QScrollArea()
        scroll.setStyleSheet("background:rgb(175, 201, 182);")
    ...
    def create_upper_label_sum(self):
        ...
        if self.expenses_total != 0:
            ...
            self.b3.setText(" Checked €:   " + str(round(self.show_sum, 2)) + " (" + str(0.0) + "%)")
        self.b3.setStyleSheet("background:rgb(175, 201, 182); color:rgb(0,100,0);")
    ...
    def create_upper_label_savings(self):
        self.b4 = QLabel()
        self.b4.setText(" Savings €:   " + str(round(self.show_savings_sum, 2)))
        self.b4.setStyleSheet("background:rgb(175, 201, 182); color:rgb(0,100,0);")
    ...
    def create_drag_and_drop(self):
        ...
        self.lst.setStyleSheet("background:rgb(175, 201, 182);")
    ...
    def create_menu_bar(self):
        ...
        clear_action = QAction(QIcon("../doc/clear.png"), "&Clear", self)
        menuFile.addAction(clear_action)

        info_action = QAction(QIcon("../doc/info.png"), "&Info", self)
        menuFile.addAction(info_action)

        delete_action = QAction(QIcon("../doc/delete.png"), "&Delete", self)
        menuFile.addAction(delete_action)

        quit_action = QAction(QIcon("../doc/quit.png"), "&Quit", self)
    ...
    def clear_window(self):
        ...
        empty.setStyleSheet("background:rgb(175, 201, 182);")
    ...
    def create_info(self):
        info = QPlainTextEdit()
        info.setStyleSheet("background:rgb(175, 201, 182);")
        ...
        info.insertPlainText("1) Drag and drop bank files in csv format to below window to graph a pie chart.\n\n"
                             ...
                             "4) Click \"File\" from above menu bar in order to do additional operations:\n"
    ...
    def create_expenses_piechart(self):
        ...
        for key, values in self.expenses.items():
            ...
            angle = round(float(values[0] * 5760) / self.expenses_total)  # Max span is 5760 -> have to calculate corresponding span angle
        ...
        if self.file_imported:
            if not self.other_process:
                graph_view.setStatusTip("Expenses piechart.")
                self.update_warning_display("Expenses piechart created successfully.")  # MESSAGE
        else:
            if not self.other_process:
    ...
    def create_income_piechart(self):
        ...
        if self.file_imported:
            if not self.other_process:
                graph_view.setStatusTip("Income piechart.")
                self.update_warning_display("Income piechart created successfully.")  # MESSAGE
        else:
            if not self.other_process:
                graph_view.setStatusTip("Load files to graph a diagram.")
                self.update_warning_display("Load files before graphing a pie chart.")  # MESSAGE
