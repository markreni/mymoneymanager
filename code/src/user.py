...
class User:
    ...
    def get_income(self) -> dict[str, list]:
        return self.file_manager._show_income()

    def get_expenses(self) -> dict[str, list]:
        return self.file_manager._show_expenses()

    def import_file(self, file_path: str) -> tuple[bool, str]:
        return self.file_manager._read_file(file_path)

    def set_importance(self, key_value: str) -> str:
        imp_success = self.file_manager._set_importance(key_value)
        if imp_success:
    ...
    def unset_importance(self, key_value: str) -> str:
        unimp_success = self.file_manager._unset_importance(key_value)
        if unimp_success:
    ...
    def set_grouping(self, grouping_list: list, group_title: str) -> tuple[str, bool]:
        if len(grouping_list) >= 1 and group_title != "":
            grouping_success = self.file_manager._set_grouping(grouping_list, group_title)
    ...
    def delete_row(self, delete_list: list) -> str:
        if len(delete_list) == 0:
            ...
            delete_success = self.file_manager._delete_row(delete_list)
